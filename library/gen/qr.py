...
def replace(
    content: str, subs: typing.List[typing.Tuple[str, typing.Callable]], bracketed: bool = True,
) -> str:
        ...
        for open, close, name in [
            ...
            if isinstance(self.rule, rules.QRuleSingle):
                subs += [
                    ...
                    (f"{variable}.points_as_{name}", lambda: open + ", ".join([
                        open + ", ".join([f"{c}" for c in p]) + close
                        for p in self.rule.points
                    ]) + close),
                    (f"{variable}.points_as_flat_{name}", lambda: open + ", ".join([
                        f"{c}" for p in self.rule.points for c in p]) + close),
                    (f"{variable}.weights_as_{name}", lambda: open + ", ".join([
                ...
                subs += [
                    ...
                    (f"{variable}.first_points_as_{name}", lambda: open + ", ".join([
                        open + ", ".join([f"{c}" for c in p]) + close
                        for p in self.rule.first_points
                    ]) + close),
                    (f"{variable}.first_points_as_flat_{name}", lambda: open + ", ".join([
                        f"{c}" for p in self.rule.first_points for c in p]) + close),
                    (f"{variable}.second_points_as_{name}", lambda: open + ", ".join([
                        open + ", ".join([f"{c}" for c in p]) + close
                        for p in self.rule.second_points
                    ]) + close),
                    (f"{variable}.second_points_as_flat_{name}", lambda: open + ", ".join([
...
class Domain(Substitutor):
    ...
    def substitute(self, code: str, variable: str, bracketed: bool = True) -> str:
        ...
        return replace(code, [
            (f"{variable}.index", lambda: f"{self.index}"),
            (f"{variable}.PascalCaseName", lambda: "".join(
                i[0].upper() + i[1:].lower() for i in parts)),
            (f"{variable}.camelCaseName", lambda: parts[0].lower() + "".join(
                i[0].upper() + i[1:].lower() for i in parts[1:])),
            (f"{variable}.snake_case_name", lambda: "_".join(i.lower() for i in parts)),
